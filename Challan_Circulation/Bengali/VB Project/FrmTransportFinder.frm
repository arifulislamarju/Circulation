...
Begin VB.Form FrmTransportFinder 
   Caption         =   "Finder - Transport"
   ClientHeight    =   7035
   ClientLeft      =   60
   ClientTop       =   345
   ClientWidth     =   6405
   ...
   Begin VB.CommandButton comdselect 
      Caption         =   "Select"
      Default         =   -1  'True
      Height          =   375
      Left            =   3915
   ...
   End
End
Attribute VB_Name = "FrmTransportFinder"
...
Private Sub comdselect_Click()
  If cnt = 1 Then  '1
            FrmBengolyCirChallan.TbTransportFrom.Text = Trim(DataGrid1.Columns(0).Text)
        End If
        If cnt = 1 Then  '2
            FrmBengolyCirChallan.TxtTransportFrom.Text = Trim(DataGrid1.Columns(1).Text)
        End If


        If cnt = 2 Then  '1
            FrmBengolyCirChallan.TbTransportTo.Text = Trim(DataGrid1.Columns(0).Text)
        End If
        If cnt = 2 Then  '2
            FrmBengolyCirChallan.TxtTransportTo.Text = Trim(DataGrid1.Columns(1).Text)
        End If
 ...
 cmdfindnow.Enabled = False
      ...
      '====================[Activating COMBO FINDBY]=========================
       With combofindby
                  ...
                  .AddItem ("Transport ID")
                  .AddItem ("Transport Name")
       End With
  ...
  Set rs = Get_RS("SELECT * FROM F14_TransportFinder")
  Set DataGrid1.DataSource = rs
     FrmTransportFinder.DataGrid1.Columns(0).Width = 1000
    FrmTransportFinder.DataGrid1.Columns(1).Width = 2500
    FrmTransportFinder.DataGrid1.Columns(0).Caption = "Transport ID"
    FrmTransportFinder.DataGrid1.Columns(1).Caption = "Transport Name"
...
Dim rs As New ADODB.Recordset
  ...
  '      '[FOR Starts With]
  '*******************************************************************
        ' [Query For Transport ID]

 If combofindby.Text = "Transport ID" And Combo1.Text = "Starts With" Then

        .Open "select * from DBO.F14_TransportFinder" _
            & " where [Division ID] like '" & Trim(tbfilter) & "%" & "' "

         ' [Query For AreaCat Name]

ElseIf combofindby.Text = "Transport Name" And Combo1.Text = "Starts With" Then
        .Open "select * from DBO.F14_TransportFinder" _
            & " WHERE [Division Name] like '" & Trim(tbfilter) & "%" & "' "
  ...
  ' [Query For TransportCat ID]

 ElseIf combofindby.Text = "Transport ID" And Combo1.Text = "Contents" Then

        .Open "select * from DBO.F14_TransportFinder" _
            & " where [Division ID] like '%" & Trim(tbfilter) & "%" & "' "

         ' [Query For TransportCat Name]

ElseIf combofindby.Text = "Transport Name" And Combo1.Text = "Contents" Then
        .Open "select * from DBO.F14_TransportFinder" _
            & " WHERE [Division Name] like '%" & Trim(tbfilter) & "%" & "' "
